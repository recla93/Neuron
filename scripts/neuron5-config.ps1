...
.DESCRIPTION
    The neuron5-only twin of scripts\configuration.ps1 (which still targets the
    classic v4 'neuron' identity). Reuses every reusable piece of that script
    (JSON helpers, menu framework, install stages) but is wired to the v5
    "Synapse" slug throughout, via _neuron_paths.ps1's Get-NeuronPaths - same
    install dir, same MCP registration key ('neuron5', not 'neuron'), same
    Start Menu folder, same uninstall safety guard - so it never touches a
    side-by-side v4 install by mistake.
    ...
    Launched by Neuron5Config.bat. Safe to run repeatedly (idempotent).
.EXAMPLE
